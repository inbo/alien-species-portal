...
test_that("Load data", {
    ...
    # Translations
    translations_dutch <- loadMetaData(language = "nl")
    translations_french <- loadMetaData(language = "fr")
    translations_english <- loadMetaData(language = "en")
...
test_that("Load full_timeseries", {
    ...
    expect_s3_class(timeseries, "ArrowObject")
  })
...
test_that("management data", {
    ...
    expect_true(all(c(
            ...
            "Vespa_velutina_shape.RData", 
            "Ondatra_zibethicus.csv") %in% 
          tmpTable$Key))
